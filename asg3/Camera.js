// Camera.js
// Uses global Matrix4 and Vector3 from cuon-matrix-cse160.js
...
export default class Camera {
  constructor(canvas) {
    ...
    this.yawDeg = (Math.atan2(fz, fx) * 180) / Math.PI; // e.g., (0,-1) => -90 deg
  ...
  // level forward for movement (ignores pitch)
  _moveForwardVec() {
    const yawRad = (this.yawDeg * Math.PI) / 180;
  ...
  // mouse pitch (up/down)
  pitch(deltaDeg) {
    this.pitchDeg = Math.max(-this.pitchLimit, Math.min(this.pitchLimit, this.pitchDeg + deltaDeg));
    this._recalcAt();
    this._updateView();
  }
}
